namespace Securibox.FacturX.Schematron.Xslt
{
    ...
    public class DynamicXPathVariables : DynamicObject
    {
        ...
        public static object BuildDynamicProps(Dictionary<XmlQualifiedName, object> lets)
        {
            ...
            var properties = _propertyCache.GetOrAdd(dynamicType, type =>
                lets.Keys
                    .Select(k => new { Name = k.ToString(), Prop = type.GetProperty(k.ToString()) })
        ...
        private static TypeBuilder CreateTypeBuilder(string typeName)
        {
            ...
            return moduleBuilder.DefineType(typeName, TypeAttributes.Sealed | TypeAttributes.Public);
        }

        private static void CreateAutoProperty(TypeBuilder typeBuilder, string propertyName, Type propertyType)
        {
